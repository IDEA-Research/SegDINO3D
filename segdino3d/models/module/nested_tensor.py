...
class NestedTensor(object):
    ...
    def __init__(self, tensors: torch.Tensor, mask: Union[torch.Tensor,
                                                          str]) -> None:
    ...
    def to_img_list_single(self, tensor: torch.Tensor,
...
def nested_tensor_from_tensor_list(tensor_list: List[torch.Tensor], fixed_img_size=None):
    # if fixed_img_size is set, pad the output img to this size
    if fixed_img_size is not None:
        if isinstance(fixed_img_size, (list, tuple)):
            assert len(fixed_img_size) == 2, "image size should be a tuple or list with two elements"
    ...
    if tensor_list[0].ndim == 3:
        ...
        if fixed_img_size is not None:
            ...
            assert orig_h <= fixed_img_size[0] and orig_w <= fixed_img_size[1], "the fixed output image size should be larger than original image"
