...
class Res16UNetBase(BaseModule):
    ...
    def __init__(self,
        ...
        if self.mode_fuse_2d_feat == "early_fusion":
            pass
        elif self.mode_fuse_2d_feat == "only_rgb":
            in_channels = 3
        else:
            raise NotImplementedError(f"Mode fuse 2d feat {self.mode_fuse_2d_feat} not implemented")
        super().__init__()
    ...
    def forward_wrapper(self, samples, targets, return_sp_mean_pos=False):
        coordinates, features, features_dinox = [], [], []
        for i in range(len(samples)):
            if 'elastic_coords' in targets[i]:
                coordinates.append(
                    targets[i]['elastic_coords'] * self.voxel_size)
            else:
                coordinates.append(samples[i][:, :3])
            features.append(samples[i][:, 3:])
            if "points_2dfeats" in targets[0]["extra_features"].keys():
                features_dinox.append(targets[i]["extra_features"]["points_2dfeats"])
            else:
                pass
            if self.mode_fuse_2d_feat == "early_fusion":
                features[-1] = torch.cat(
                    [features[-1], targets[i]["extra_features"]["points_2dfeats"]], dim=-1)
            elif self.mode_fuse_2d_feat == "only_rgb":
                pass
            else:
        ...
        x = self.forward(field.sparse())
        x = x.slice(field).features

        # apply scatter_mean to obtain superpoint features.
        sp_pts_masks, n_super_points = [], []
        for target in targets:
            sp_pts_mask = target["extra_features"]["super_point_masks"]  # n_points, indicating the super point id
            sp_pts_masks.append(sp_pts_mask + sum(n_super_points))
            n_super_points.append(sp_pts_mask.max() + 1)
        x = scatter_mean(x, torch.cat(sp_pts_masks), dim=0)  # get the features for each super point.
        if len(features_dinox) > 0:
            x_dinox = scatter_mean(torch.cat(features_dinox), torch.cat(sp_pts_masks), dim=0)  # get the features for each super point.
        else:
            x_dinox = None

        # split superpoint features to batch data.
        features = []
        ...
        if self.add_positional_embedding:
            x_mean_coords = scatter_mean(
                coordinates[..., 1:].float() * self.voxel_size, 
            ...
            for i in range(len(n_super_points)):
                ...
                pos.append(x_mean_coords[begin: end])
        else:
            pos = None
        ...
        if return_sp_mean_pos:
            ...
            for i in range(len(samples)):
                coordinates_wo_elastic.append(samples[i][:, :3])
                features_wo_elastic.append(samples[i][:, 3:])
            coordinates_wo_elastic, features_wo_elastic = ME.utils.batch_sparse_collate(
                [(c / self.voxel_size, f) for i, (c, f) in enumerate(zip(coordinates_wo_elastic, features_wo_elastic))],
                device=coordinates_wo_elastic[0].device)
            field = ME.TensorField(coordinates=coordinates_wo_elastic, features=features_wo_elastic)
            ...
            pos_wo_elastic = None
        return features, pos, pos_wo_elastic
